...
const mockIdProcessor = require("../../exampleCode/codePatterns/idProcessor.js");
...
/* Mocks & Objects */
jest.mock("axios");
jest.mock("../../exampleCode/codePatterns/idProcessor.js");
...
const runtimeCtx = new RuntimeContext(AsyncMode.Serial, TimeUnit.millisecond, 0.05);
...
runtimeCtx.mockWithModel(mockIdProcessor.add, "add", randPerfModel)
...
describe("registrar", () => {
  ...
  test("Sync test code with sync and async mocks", async () => {
    ...
    expect(mockAxios.get).toHaveBeenCalledTimes(runs * 2);
    expect(mockAxios.put).toHaveBeenCalledTimes(runs * 2);
    expect(mockIdProcessor.add).toHaveBeenCalledTimes(runs * 3);
    expect(runtimeCtx.runtimeMean()).toBeLessThan(10);
  });
  ...
  test("Sync test code with mocks and real time delay", async () => {
    ...
    expect(mockIdProcessor.add).toHaveBeenCalledTimes(runs * 2);
    expect(runtimeCtx.runtimeMean()).toBeLessThan(10);
  });
